  ...
  return (
    <div>
      <div className="max-w-md mx-auto flex justify-center relative">
        {/* <div className="absolute -inset-1 bg-gradient-to-r from-primary-dark to-primary blur-lg opacity-30 group-hover:opacity-90 transition duration-700 group-hover:duration-300"></div> */}

        <Link
          href={link}
          className="group font-medium whitespace-nowrap tracking-wide select-none text-lg relative inline-flex items-center justify-center cursor-pointer h-12 border border-solid py-0 px-8 overflow-hidden z-10 transition-all duration-300 ease-in-out outline-0 bg-primary group-hover:bg-transparent text-white border-white group-hover:text-primary focus:text-primary"
        >
          <strong className="font-bold ">{title}</strong>
          <span>
            <svg
              className="ml-1 rotate-180 fill-white group-hover:fill-primary group-hover:translate-x-4 transition-all duration-300"
              width="27"
              height="27"
              viewBox="0 0 27 27"
              fill="none"
          ...
          <span className="absolute bg-white/85 bottom-0 w-0 left-1/2 h-full -translate-x-1/2 transition-all ease-in-out duration-300 group-hover:w-[105%] -z-[1] group-focus:w-[105%]"></span>
        </Link>
      </div>
    </div>
  );
